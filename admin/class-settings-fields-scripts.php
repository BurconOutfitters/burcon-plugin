...
class Settings_Fields_Scripts {
	...
	public function settings() {
		 ...
		 */
		add_settings_section( 'burcon-scripts-general', __( 'General Options', 'burcon-plugin' ), [ $this, 'scripts_general_section_callback' ], 'burcon-scripts-general' );

		// Inline scripts.
		add_settings_field( 'burcon_inline_scripts', __( 'Inline Scripts', 'burcon-plugin' ), [ $this, 'burcon_inline_scripts_callback' ], 'burcon-scripts-general', 'burcon-scripts-general', [ esc_html__( 'Add script contents to footer', 'burcon-plugin' ) ] );
		...
		// Inline styles.
		add_settings_field( 'burcon_inline_styles', __( 'Inline Styles', 'burcon-plugin' ), [ $this, 'burcon_inline_styles_callback' ], 'burcon-scripts-general', 'burcon-scripts-general', [ esc_html__( 'Add script-related CSS contents to head', 'burcon-plugin' ) ] );
		...
		// Inline jQuery.
		add_settings_field( 'burcon_inline_jquery', __( 'Inline jQuery', 'burcon-plugin' ), [ $this, 'burcon_inline_jquery_callback' ], 'burcon-scripts-general', 'burcon-scripts-general', [ esc_html__( 'Deregister jQuery and add its contents to footer', 'burcon-plugin' ) ] );
		...
		// Include jQuery Migrate.
		add_settings_field( 'burcon_jquery_migrate', __( 'jQuery Migrate', 'burcon-plugin' ), [ $this, 'burcon_jquery_migrate_callback' ], 'burcon-scripts-general', 'burcon-scripts-general', [ esc_html__( 'Use jQuery Migrate for backwards compatibility', 'burcon-plugin' ) ] );
		...
		// Remove emoji script.
		add_settings_field( 'burcon_remove_emoji_script', __( 'Emoji Script', 'burcon-plugin' ), [ $this, 'remove_emoji_script_callback' ], 'burcon-scripts-general', 'burcon-scripts-general', [ esc_html__( 'Remove emoji script from <head>', 'burcon-plugin' ) ] );
		...
		add_settings_field( 'burcon_remove_script_version', __( 'Script Versions', 'burcon-plugin' ), [ $this, 'remove_script_version_callback' ], 'burcon-scripts-general', 'burcon-scripts-general', [ esc_html__( 'Remove WordPress version from script and stylesheet links in <head>', 'burcon-plugin' ) ] );
		...
		// Minify HTML.
		add_settings_field( 'burcon_html_minify', __( 'Minify HTML', 'burcon-plugin' ), [ $this, 'html_minify_callback' ], 'burcon-scripts-general', 'burcon-scripts-general', [ esc_html__( 'Minify HTML source code to increase load speed', 'burcon-plugin' ) ] );
		 ...
		 */
		add_settings_section( 'burcon-scripts-vendor', __( 'Included Vendor Scripts', 'burcon-plugin' ), [ $this, 'scripts_vendor_section_callback' ], 'burcon-scripts-vendor' );

		// Use Slick.
		add_settings_field( 'burcon_enqueue_slick', __( 'Slick', 'burcon-plugin' ), [ $this, 'enqueue_slick_callback' ], 'burcon-scripts-vendor', 'burcon-scripts-vendor', [ esc_html__( 'Use Slick script and stylesheets', 'burcon-plugin' ) ] );
		...
		// Use Tabslet.
		add_settings_field( 'burcon_enqueue_tabslet', __( 'Tabslet', 'burcon-plugin' ), [ $this, 'enqueue_tabslet_callback' ], 'burcon-scripts-vendor', 'burcon-scripts-vendor', [ esc_html__( 'Use Tabslet script', 'burcon-plugin' ) ] );
		...
		// Use Sticky-kit.
		add_settings_field( 'burcon_enqueue_stickykit', __( 'Sticky-kit', 'burcon-plugin' ), [ $this, 'enqueue_stickykit_callback' ], 'burcon-scripts-vendor', 'burcon-scripts-vendor', [ esc_html__( 'Use Sticky-kit script', 'burcon-plugin' ) ] );
		...
		// Use Tooltipster.
		add_settings_field( 'burcon_enqueue_tooltipster', __( 'Tooltipster', 'burcon-plugin' ), [ $this, 'enqueue_tooltipster_callback' ], 'burcon-scripts-vendor', 'burcon-scripts-vendor', [ esc_html__( 'Use Tooltipster script and stylesheet', 'burcon-plugin' ) ] );
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			...
			add_settings_section( 'burcon-registered-fields-activate', __( 'Registered Fields Activation', 'burcon-plugin' ), [ $this, 'registered_fields_activate' ], 'burcon-registered-fields-activate' );

			add_settings_field( 'burcon_acf_activate_settings_page', __( 'Site Settings Page', 'burcon-plugin' ), [ $this, 'registered_fields_page_callback' ], 'burcon-registered-fields-activate', 'burcon-registered-fields-activate', [ __( 'Deactive the field group for the "Site Settings" options page.', 'burcon-plugin' ) ] );
	...
	public function registered_fields_activate() {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			...
			echo sprintf( '<p>%1s</p>', esc_html( 'The Controlled Chaos plugin registers custom fields for Advanced Custom Fields Pro that can be imported for editing. These built-in field goups must be deactivated for the imported field groups to take effect.', 'burcon-plugin' ) );
