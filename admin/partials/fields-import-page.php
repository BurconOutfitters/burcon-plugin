...
<div class="wrap import-registered-field-groups">
    ...
    <?php if ( $active_tab == 'burcon-registered-fields-import' ) :
        ...
        <div id="message" class="error below-h2">
            <p><?php printf( esc_html__( 'This tool was built for ACF version 5 and you have version %s.', 'burcon-plugin' ) ); ?></p>
        </div>
        <?php endif; ?>

        <?php if ( ! empty( $imported ) ) { ?>
        <div id="message" class="updated below-h2">
            <p><strong><?php esc_html_e( 'Field groups imported:', 'burcon-plugin' ); ?></strong></p>
            <ul>
        ...
        <?php }
        printf( '<p>%1s</p>', esc_html__( 'This tool imports any field groups registered outside the ACF plugin so that they can be edited.', 'burcon-plugin' ) ); ?>
        <?php if ( ! empty( $acf_local->groups ) ) : ?>

        <form method="POST">
            <table class="widefat">
                <thead>
                    <th><?php esc_html_e( 'Import', 'burcon-plugin' ); ?></th>
                    <th><?php esc_html_e( 'Registered Field Groups', 'burcon-plugin' ); ?></th>
                    <th><?php esc_html_e( 'Possible Existing Matches', 'burcon-plugin' ); ?></th>
                </thead>
                <tbody>
                    <?php
                    ...
                    <tr>
                        ...
                        if ( empty( $matches ) ) {
                            echo '<em>' . esc_html__( 'No matches found.', 'burcon-plugin' ) . '</em>';
                        } else {
                            $links = array();
            ...
            <p class="submit">
                <input type="submit" value="<?php esc_html_e( 'Import', 'burcon-plugin' ); ?>" class="button-primary" />
            </p>
        </form>

        <h3><?php esc_html_e( 'Field groups found in files:', 'burcon-plugin' ); ?></h3>
