...
final class ACF_Tab_Icons {
	...
	public function enqueue_scripts_styles() {
		...
		if ( file_exists( get_theme_file_path( 'assets/fonts/icons/afc-icons.css' ) ) ) {
			wp_register_style( 'acf-tab-icons', get_theme_file_uri( 'assets/fonts/icons/afc-icons.css' ), [], BURCON_VERSION );
	...
	public function icon_register_field_settings( $field ) {
		...
		if ( ! isset( $json_content['icons'] ) ) {
			...
			acf_render_field_setting( $field, [
				'label'			=> __( 'Icon', 'burcon-plugin' ),
				'instructions'	=> '',
				'type'			=> 'message',
				'message'		=> __( 'No icons found', 'burcon-plugin' ),
				'new_lines'		=> ''
			] );
		...
		acf_render_field_setting( $field, [
			'label'			=> __( 'Icon', 'burcon-plugin' ),
			'instructions'	=> __( 'Select an icon you want to show before the tab text.', 'burcon-plugin' ),
			'type'			=> 'select',
		...
		acf_render_field_setting( $field, [
			'label'			=> __( 'Show icon only', 'burcon-plugin'),
			'instructions'	=> __( 'If set to <em>Yes</em>, you will see only the icon and no text.', 'burcon-plugin'),
			'name'			=> 'show_icon_only',
			'type'			=> 'true_false',
			'ui'			=> 1,
		] );
