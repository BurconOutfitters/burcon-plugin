...
class Admin_Pages {
    ...
    public function about_plugin() {
        ...
        // Otherwise use Site Plugin as the label.
            ...
            $label = __( 'Site Plugin', 'burcon-plugin' );
        }
        ...
        // Otherwise use the scholar's cap icon to imply instruction.
            ...
            $icon = __( 'dashicons-welcome-learn-more', 'burcon-plugin' );
        }
    ...
    public function help_about_plugin() {
		...
		$screen->add_help_tab( [
			'id'       => 'help_plugin_info',
			'title'    => __( 'More Information', 'burcon-plugin' ),
			'content'  => null,
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Convert Plugin', 'burcon-plugin' ),
			'content'  => null,
    ...
    public function help_about_page_sidebar() {
        ...
        $html  = sprintf( '<h4>%1s</h4>', __( 'Author Credits', 'burcon-plugin' ) );
        $html .= sprintf(
            '<p>%1s %2s.</p>',
            __( 'This plugin was originally written by', 'burcon-plugin' ),
            'Greg Sweet'
        );
        $html .= sprintf(
            '<p>%1s <br /><a href="%2s" target="_blank">%3s</a> <br />%4s</p>',
            __( 'Visit:', 'burcon-plugin' ),
            'http://ccdzine.com/',
            'Controlled Chaos Design',
            __( 'for more free downloads.', 'burcon-plugin' )
        );
        $html .= sprintf(
            '<p>%1s</p>',
            __( 'Change this sidebar to give yourself credit for the hard work you did customizing this plugin.', 'burcon-plugin' )
         );
    ...
    public function title_placeholders( $title ) {
        ...
        // Post type: page.
            ...
            $post_title = esc_html__( 'Page Title', 'burcon-plugin' );
        ...
        // Post type: custom, unidentified.
            ...
            $post_title = esc_html__( 'Enter Title', 'burcon-plugin' );
        }
